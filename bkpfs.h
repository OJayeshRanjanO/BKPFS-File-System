...
extern struct dentry *bkpfs_lookup(struct inode *dir, struct dentry *dentry,unsigned int flags);
extern struct inode *bkpfs_iget(struct super_block *sb,struct inode *lower_inode);
extern int bkpfs_interpose(struct dentry *dentry, struct super_block *sb, struct path *lower_path);

//MY CODE
extern int backup_helper(struct inode *inode, struct file *file);
extern int create_negative_dentry (struct dentry * dir_dentry, char * name, struct dentry ** negative_dentry);
extern int create_file(struct inode *dir, struct vfsmount *mnt, struct dentry *lower_dentry,umode_t mode, bool want_excl);
extern int write_to_backup(struct file *src_file, struct inode *dir, struct vfsmount *mnt, struct dentry *dst_dentry);
extern int update_meta_file(char *backup_name, const char * filename, struct path lower_path, int flags, int mode);
extern int list_files(struct file *file, char **buffer);
extern int view_version(struct file *file, int version, char **buffer);
extern int get_nth_line(struct file *file, int n, char **buffer);
extern int get_backup_size(struct file * meta_file);
extern int delete_version(struct file *file, int version);
extern int negate_version(struct file *meta_file, int version, char ** buffer);
extern int restore_version(struct file *meta_file, struct file *original_file, int version);
extern int max_versions; // Number of version
extern int backup_on;
