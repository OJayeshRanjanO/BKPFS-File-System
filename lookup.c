				      ...
				      struct path *lower_parent_path)
{
	...
	if (dentry!= NULL){
		printk("Dentry: %s, Path: %s ",dentry->d_name.name,lower_parent_path->dentry->d_name.name);
	}
	if (IS_ROOT(dentry))
		goto out;
	...
	printk("lower_dir_dentry: %s mnt: %s name: %s ",lower_dir_dentry->d_name.name, lower_dir_mnt->mnt_root->d_name.name,name);
	//printk("lower_path: %s",lower_path.dentry->d_name.name);
			     ...
			     unsigned int flags)
{
	...
	struct path lower_parent_path;
        UDBG;
	printk("Parent Dentry %s, file dentry %s",dentry->d_parent->d_name.name,dentry->d_name.name);
	if (memcmp(dentry->d_name.name,"bkpfs",5) == 0){
		printk("------------ I should never reach here------------------");
	...
	if (err) {
		ret = ERR_PTR(err);
		printk("err1: %lu",PTR_ERR(ret));
		goto out;
	}
	ret = __bkpfs_lookup(dentry, flags, &lower_parent_path);

	if (IS_ERR(ret)){
		printk("err2: %lu",PTR_ERR(ret));	
		goto out;
	}
	if (ret){
		printk("err3: %lu",PTR_ERR(ret));
		dentry = ret;
	}
	if (d_inode(dentry)){
		fsstack_copy_attr_times(d_inode(dentry),bkpfs_lower_inode(d_inode(dentry)));
