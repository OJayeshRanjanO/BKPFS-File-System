			    ...
			    const char *dev_name, void *raw_data)
{
	...
	sscanf ( (const char *)raw_data, "maxver=%d" , &max_versions);
	if (max_versions == 0){ // Number of backups to keep (Can be configured)
		max_versions = 5;
	}
	if (backup_on == 0){ // Backup only if more than 5 bytes are written (Can be configured)
		backup_on = 10;
	}
	return mount_nodev(fs_type, flags, lower_path_name, bkpfs_read_super);
}
